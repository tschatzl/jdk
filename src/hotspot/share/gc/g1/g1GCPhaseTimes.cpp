  ...
  _weak_phase_times(max_gc_threads)
{
  ...
  _gc_par_phases[FlushPss] = new WorkerDataArray<double>("MergePSS", "Merge Per-Thread State (ms):", max_gc_threads);
  ...
  _gc_par_phases[UpdateCodeRoots] = new WorkerDataArray<double>("UpdateCodeRootsTask", "Update Code Roots (ms):", _max_gc_threads);
...
double G1GCPhaseTimes::print_post_evacuate_collection_set(bool evacuation_failed) const {
  ...
  debug_phase(_gc_par_phases[DestroyPss]);
