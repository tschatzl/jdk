...
public:
  virtual bool do_heap_region(HeapRegion* r) {
    ...
    if (part_of_marking) {
      guarantee(r->bottom() != r->top_at_mark_start(), "region %u (%s) does not have TAMS set", r->hrm_index(), r->get_short_type_str());
      size_t marked_bytes = cm->live_bytes(r->hrm_index());

      MarkedBytesClosure cl;
      r->apply_to_marked_objects(cm->mark_bitmap(), &cl);

      guarantee(cl.marked_bytes() == marked_bytes, "region %u (%s) live bytes actual %zu and cache %zu differ", r->hrm_index(), r->get_short_type_str(), cl.marked_bytes(), marked_bytes);
    } else {
      guarantee(r->bottom() == r->top_at_mark_start(), "region %u (%s) has TAMS set " PTR_FORMAT " " PTR_FORMAT, r->hrm_index(), r->get_short_type_str(), p2i(r->bottom()), p2i(r->top_at_mark_start()));
      guarantee(cm->live_bytes(r->hrm_index()) == 0, "region %u (%s) has %zu live bytes recorded", r->hrm_index(), r->get_short_type_str(), cm->live_bytes(r->hrm_index()));
      guarantee(cm->mark_bitmap()->get_next_marked_addr(r->bottom(), r->end()) == r->end(), "region %u (%s) has mark", r->hrm_index(), r->get_short_type_str());
      guarantee(cm->is_root_region(r), "region %u (%s) should be root region", r->hrm_index(), r->get_short_type_str());
    }
    return false;
  }
};
