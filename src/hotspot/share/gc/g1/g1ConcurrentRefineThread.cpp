  ...
  while (wait_for_completed_buffers()) {
    ...
    while (!should_terminate()) {
      if (sts_join.should_yield()) {
        ...
        if (!cr()->adjust_threads_periodically() || (!cr()->is_thread_wanted()) || cr()->wait_for_heap_lock()) {
          log_debug(gc,refine)("Concurrent Refine Adjust Only (requested active: %s threads wanted: %u adjustment needed: %s wait for heap lock: %s) %.2fms",
                               BOOL_TO_STR(_requested_active),
                               cr()->threads_wanted(),
...
bool G1ConcurrentRefineThread::wait_for_completed_buffers() {
  assert(this == Thread::current(), "precondition");
...
void G1ConcurrentRefineThread::do_refinement() {
  ...
  log_debug(gc,refine)("Concurrent Refine Work Start (threads wanted: %u)", _cr->threads_wanted());
