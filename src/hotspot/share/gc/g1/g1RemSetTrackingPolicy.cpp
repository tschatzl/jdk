...
G1RemSetTrackingPolicy::Result G1RemSetTrackingPolicy::update_at_allocate(G1HeapRegion* r) {
...
G1RemSetTrackingPolicy::Result G1RemSetTrackingPolicy::update_at_free(G1HeapRegion* r) {
  return Remove;
}

G1RemSetTrackingPolicy::Result G1RemSetTrackingPolicy::update_humongous_before_rebuild(G1HeapRegion* r) {
...
G1RemSetTrackingPolicy::Result G1RemSetTrackingPolicy::update_old_before_rebuild(G1HeapRegion* r) {
  assert(SafepointSynchronize::is_at_safepoint(), "should be at safepoint");
...
G1RemSetTrackingPolicy::Result G1RemSetTrackingPolicy::update_after_rebuild(G1HeapRegion* r) {
  assert(SafepointSynchronize::is_at_safepoint(), "should be at safepoint");
  ...
  if (r->is_old_or_humongous()) {
    ...
    if (r->rem_set()->has_single_region_card_rem_set()) {
        G1CardRemSet *card_set = r->rem_set()->card_rem_set();
