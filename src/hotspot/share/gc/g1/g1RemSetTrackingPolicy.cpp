...
static void print_before_rebuild(HeapRegion* r, bool selected_for_rebuild, size_t total_live_bytes, size_t live_bytes) {
  log_trace(gc, remset, tracking)("Before rebuild region %u "
                                  ...
                                  "type %s)",
                                  r->hrm_index(),
                                  p2i(G1CollectedHeap::heap()->concurrent_mark()->top_at_mark_start(r->hrm_index())),
                                  total_live_bytes,
...
bool G1RemSetTrackingPolicy::update_before_rebuild(HeapRegion* r, size_t live_bytes_below_tams, size_t bytes_above_tams) {
  assert(SafepointSynchronize::is_at_safepoint(), "should be at safepoint");
  ...
  size_t total_live_bytes = live_bytes_below_tams + bytes_above_tams;
...
void G1RemSetTrackingPolicy::update_after_rebuild(HeapRegion* r) {
  ...
  if (r->is_old_or_humongous_or_archive()) {
    ...
    log_trace(gc, remset, tracking)("After rebuild region %u "
                                    ...
                                    "size %zu)",
                                    r->hrm_index(),
                                    p2i(cm->top_at_mark_start(r->hrm_index())),
