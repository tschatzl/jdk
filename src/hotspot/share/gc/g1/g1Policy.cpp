...
void G1Policy::record_dirtying_stats(double last_mutator_start_dirty_ms,
  ...
  assert(SafepointSynchronize::is_at_safepoint() || G1ReviseYoungLengthTargetLengthTask->is_locked(),
         "must be (at safepoint %s locked %s)",
         BOOL_TO_STR(SafepointSynchronize::is_at_safepoint()), BOOL_TO_STR(G1ReviseYoungLengthTargetLengthTask->is_locked()));
