...
size_t G1HeapSizingPolicy::young_collection_shrink_amount(double delta) const {
  assert(delta >= 0.0, "must be");
...
size_t G1HeapSizingPolicy::young_collection_resize_amount(bool& expand) {
  ...
  if ((_ratio_exceeds_threshold == MinOverThresholdForExpansion) ||
    ...
    resize_bytes = young_collection_shrink_amount(delta);
    expand = false;
